...
namespace Moquestra.TypeIds.SourceGenerator
{
    ...
    public sealed class TypeIdGenerator : IIncrementalGenerator
    {
        ...
        private static void Emit(SourceProductionContext context, ImmutableArray<TypeIdCandidate> candidates, ImmutableArray<(string Namespace, string Name, Location Location)> conflicts, (string Namespace, string? SanitizedFrom) mapNamespace, ImmutableArray<MapNameDesignation> mapNames)
        {
            ...
            var mapFiles = new List<(string FullName, string Namespace, string ClassText)>();
            ...
            foreach (var domain in domains)
            {
                ...
                mapFiles.Add((
                    ...
                    targetNamespace,
                    BuildMapClass(targetNamespace, className, domain, accepted, BuildConstantNames(context, targetNamespace, className, accepted))));
            }

            // Generated file names must be unique regardless of casing.
            var hintCounts = new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase);

            foreach (var file in mapFiles)
            {
                hintCounts.TryGetValue(file.FullName, out var count);
                hintCounts[file.FullName] = count + 1;
            }

            foreach (var file in mapFiles)
            {
                var hintName = hintCounts[file.FullName] > 1
                    ? file.FullName + "." + Hashing.Fnv1a.Compute(Encoding.UTF8.GetBytes(file.FullName)).ToString("x8", CultureInfo.InvariantCulture)
                    : file.FullName;
